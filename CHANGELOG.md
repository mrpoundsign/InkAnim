...
### Added
- **In-Memory SVG Text-to-Path Vector Converter**: Added in-memory conversion of SVG `<text>` and `<tspan>` elements (including CSS SVG 2 flowed text `shape-inside`) into standard `<path>` vector glyph contours before rasterization without destructively flattening files on disk. Features system TrueType/OpenType font resolution, cross-platform fallback to Fyne embedded fonts, kerning, uniform letter-spacing, and integration with ancestor group transform stroke scaling and paint-order desugaring ([#21](https://github.com/mrpoundsign/InkAnim/issues/21)).
