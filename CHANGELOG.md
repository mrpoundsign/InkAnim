...
### Added
- **Embedded Cross-Platform DejaVu Sans Fonts**: Embedded official open-source DejaVu Sans (Regular & Bold) TrueType fonts into `internal/svg` to guarantee deterministic, pixel-perfect text rendering across Windows, macOS, Linux, and WebAssembly, resolving OS-level font metric discrepancies against canonical Linux/Inkscape golden references ([#69](https://github.com/mrpoundsign/InkAnim/issues/69)).
...
### Changed
- **Modularized SVG Engine (`pkg/inksvg`)**: Extracted the in-memory SVG preprocessor, parser, and rasterizer from `internal/svg` into a public, decoupled package `pkg/inksvg` with zero GUI/Fyne or internal dependencies, preparing for future standalone library extraction and multi-project reuse ([#50](https://github.com/mrpoundsign/InkAnim/issues/50)).
- **Updated Go Dependencies & Security Patches**: Updated indirect and direct Go module dependencies to resolve security advisories in `golang.org/x/net`, upgraded `golang.org/x/image`, `golang.org/x/sys`, `golang.org/x/text`, and tidied unused module definitions ([#71](https://github.com/mrpoundsign/InkAnim/issues/71)).
